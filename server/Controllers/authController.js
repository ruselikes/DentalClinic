...
class authController{
    ...
    async registration(req,res){
        ...
        try{
            ...
            const hashedPassword =await bcrypt.hash(password,2)
            const new_user = new User ({email:email,password: hashedPassword,role:role,userId})
            await new_user.save()
            if (req.body.role==="admin"){
                res.status(201).json({message:"Админ добавлен систему",user:new_user})
            }
            else if (req.body.role==="pacient"){
                res.status(201).json({message:"Пациент добавлен систему",user:new_user})
            }
            else if (req.body.role==="staff"){
                res.status(201).json({message:"Регистратор добавлен систему",user:new_user})
            }
            else if (req.body.role==="doctor"){
                res.status(201).json({message:"Доктор добавлен систему",user:new_user})
            }
        }
        catch(e){
            res.status(500).json({message:"На моем серверe (при регистрации) что то не так. tg: trimberg",error: e.message})
    ...
    async login(req,res){
        try{
            ...
            console.log({token})
            // localStorage.setItem('userData', JSON.stringify(token));
            return res.status(200).json({token:token,id:user.userId})
